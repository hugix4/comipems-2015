...
$Npreguntas=10;
$responderD='S';//Activar o dasactivar que se responda todo como D con un S o N
$nombreBD='comipems';
...
if (empty($_POST['min'])) { $minTotal=59;} else { $minTotal=$_POST['min'];}
//$minTotal=$_POST['min'];
if($minTotal==''){
...
if (empty($_POST['seg'])) { $segTotal=60;} else { $segTotal=$_POST['seg'];}
	...
	<head>
		...
		<script>
		  $(document).ready(function(){
			...
			function muestraMin(minutos){
				//alert("Los valores que se obtuvieron en muestraMin fueron "+minutos);
				if(minutos<=-1){
					//alert("Los minutos ya terminaron");
					return;
					
				}
					document.getElementById('mins').value='Tiempo restante: ' +minutos+'m';
					document.getElementById('min').value=minutos;
			}
			
			function muestraSeg(){//<?php echo $minTotal;?>, <?php echo $segTotal;?>,  <?php echo $vueltaTotal;?>
				alert("Entraste al muestraSeg");
				//alert("Los valores que se obtuvieron en muestraSeg fueron "+minutos+", "+segundos+" y vuelta:"+vuelta);			
				/*segundos=segundos-1;
				if(segundos==-1 && vuelta<60){
					...
					minutos=minutos-1;
					//alert("Minutos despues "+minutos+" y  vuelta: "+vuelta);
					muestraMin(minutos);				
					//alert("Valor de vuelta es "+vuelta);
				}
				if(vuelta==60){
				...
				document.getElementById('segs').value=': '+segundos+'s';
				document.getElementById('seg').value=segundos;
				document.getElementById('vuelta').value=vuelta;
				setTimeout(function(){muestraSeg(minutos,segundos,vuelta)},1000); // 1000ms = 1s*/
			}
			...
			function hola(minuto,seg,vuelta){
				...
				document.getElementById('vuelta').value=vuelta;
				setTimeout(function(){hola(minuto,seg,vuelta) },100); // 1000ms = 1s
			}
			
			/*function repite(){
				alert("Hola desde el repite()");
				 setTimeout(function(){ repite() }, 3000);
			}*/
				...
				<?php
					...
					function cadenaBin($Npreguntas,$rBinarias,$Nactual){
						...
						{					
							...
							echo "<br/><br/>			
							<u>
								<a href='salirC.php' style='color:yellow'>
						...
						{//$_POST['Et']==null){
							...
							{	
								...
								if($Folio==987654321 || $Folio==987654322 || $Folio==987654323 || $Folio==987654325 || $Folio==987654327)
								{
									$sql="Update $nombreBD Set Calificacion='$calNum',Resultado='$colocacion',Seccion='$Seccion',  Termino='N',Etapa='0' where Folio='$Folio";							
									echo $sql;
									//$sqlD=$conexionD->consulta($sql);
									$sql=$conexion->consulta($sql);	
									echo "<br/><br/><u><a href='salirC.php' style='color:black'>Salir</a></u>";
									echo"<br/><br/>Examen terminado. Tu examen de inglés ha finalizado satisfactoriamente<br/><br/>";
									//echo "<br/><br/><u><a target='_blank' href='calificaS.php?c=$_SESSION[Folio]&cal=$calNum&rs=$colocacion' style='color:black'>Calificacion en pdf</a></u>";
								}
								else
								{
									$periodo = '1516';
									//echo "Antes de la consulta WSEDI no hay nada";
									$respuestaEDI = consultaWSEDI($Folio, $periodo);
									//echo $respuestaEDI;
						...
						{					
							...
							{						
								...
								if($Pregunta=='' || $OpA=='' || $OpB=='' || $OpC=='' || $OpD=='' || $rBien==''){
									//echo"Hubo un error con la pregunta<br/>";
									//echo"Se volverá a llamar al proceso";
									do{
										$sqlQ = "select Pregunta, A, B, C, D, Respuesta from $bdPreguntas where Numero='$Num'";
										$sqlQ=$conexion->consulta($sqlQ);
										//$rowP=mysql_fetch_array($sqlQ);
										while($rowQ=mysql_fetch_array($sqlQ))
										{
											$Pregunta=$rowQ["Pregunta"];
											$OpA=$rowQ["A"];
									...
									}while($Pregunta=='' || $OpA=='' || $OpB=='' || $OpC=='' || $OpD=='' || $rBien=='');
								}
								...
								if($i>=1 && $i<=$numPreguntas){							
									...
									//echo"Ocurrió algo imprevisto, se forzará la aparición de las preguntas";
									if($i>=1 && $i<=$numPreguntas){							
									preguntaDe4($i,$Pregunta, $OpA, $OpB, $OpC, $OpD,$rBien,($etapa+1));
									}
								}
