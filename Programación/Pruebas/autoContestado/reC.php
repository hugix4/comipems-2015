...
<html lang="es">
	<head>
		...
		<script>
		  $(document).ready(function(){
			...
			function autoC() {
			//alert("Entró al  autoC");				
				  setTimeout(function() {document.getElementById('btnC').click();}, 300);
				
			  setTimeout(function() {document.getElementById('btnT1').click();}, 300);
			  
			  setTimeout(function() {document.getElementById('btnT').click();}, 300);
			  
			  setTimeout(function() {document.getElementById('boton').click();}, 310);
	...
	<body  style="margin-top: 0%; font-size:1.4em" onload='hola(<?php echo $minTotal;?>,<?php echo $segTotal;?>,<?php echo $vueltaTotal;?>); autoC();' >	
				...
				<?php
					...
					function cadenaBin($Npreguntas,$rBinarias,$Nactual){
						...
						{					
							...
							<br/>
							";
							echo"<br/><br/><form action='salirC.php'><input class='btn' id='btnC' type='submit' value='Cerrar Sesión' /></form>";	
							echo"<script>	autoC();			
									</script>";							
						}						
						else
						{
								...
								{	
									...
									//echo"Update $nombreBD Set Calificacion='$calNum',Resultado='$colocacion', Termino='S' where Folio='$_SESSION[Folio]'";
										...
										if($respuestaEDI[0]==true){
											...
											echo"<br/><br/><form action='salirC.php'><input class='btn' type='submit' id='btnC' value='Cerrar Sesión' /></form>";
											echo"<script>	autoC();</script>";
							...
							{	
								...
								{	
									...
									//$sqlD=$conexionD->consulta($sql);
									$sql=$conexion->consulta($sql);				
									echo"<br/><br/>Da clic en el botón para poder finalizar tu examen";
									
									echo"<br/><br/><form action='$pExamen'>																		
									<input class='btn' type='submit' id='btnT1' value='Terminar examen' style='margin-left:150;'/>
									</form>";
							...
							{	
								...
								while($rowQ=mysql_fetch_array($sqlQ))
								{
									...
									/*if($Pregunta=='' || $OpA=='' || $OpB=='' || $OpC=='' || $OpD=='' || $rBien==''){
										...
										do{
											$sqlQ = "select Pregunta, A, B, C, D, Respuesta from $bdPreguntas where Numero Between '$pInicial' and '$pFinal";
											$sqlQ=$conexion->consulta($sqlQ);
								...
								if($responderD=='S')
								{
								echo "<br/><br/><input class='btn' type='button' id='btnT' onclick=todas(1) value='Responder todo D' style='margin-left:150;'>";
								}
